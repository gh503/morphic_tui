    ...
    fn render(&self, frame: &mut Frame, area: Rect, _config: &AppConfig) {
        ...
        let config_text = vec![
            ...
            Line::from(vec![
                ...
                Span::raw(" : 调整监控图表横轴长度 (10-200)"),
            ]),
            Line::from(vec![
                Span::styled("  Tab   ", Style::default().fg(Color::Yellow)),
                Span::raw(" : 切换回监控应用查看效果"),
            ]),
        ];

        frame.render_widget(
            Paragraph::new(config_text).block(Block::bordered().title(" 核心参数 ").border_type(BorderType::Rounded).border_style(Style::default().fg(Color::Magenta))),
            chunks[0]
        );
        ...
        let info_text = vec![
            Line::from(" [ 采样机制说明 ] ").add_modifier(Modifier::UNDERLINED),
            Line::from(vec![
                Span::raw(" • 覆盖时长: "),
                Span::styled(format!("{:.1} 秒", duration), Style::default().fg(Color::White).bold()),
                Span::raw(format!(" (公式: {} 点 × 0.2s)", self.current_points)),
            ]),
            Line::from(vec![
                Span::raw(" • 当前模式: "),
            ...
            Line::from(""),
            Line::from(" [ SDET 性能视点 ] ").add_modifier(Modifier::UNDERLINED),
            Line::from(" • 数据源: 基于 /proc/stat 的差值采样"),
            Line::from(" • 渲染压力: 随采样点线性增长 (Braille 开销)"),
            Line::from(" • 内存状态: 环形队列存储，内存抖动极低"),
        ];

        frame.render_widget(
            Paragraph::new(info_text).block(Block::bordered().title(" 实时分析 ").border_style(Style::default().fg(Color::DarkGray)).border_type(BorderType::Rounded)),
            chunks[1]
        );
        ...
        let tips_text = vec![
            Line::from(vec![
                Span::styled(" 💡 交互小技巧 (Terminal Tips) ", Style::default().fg(Color::Yellow).bold()),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::raw(" • "),
                Span::styled("强制选择/复制: ", Style::default().bold()),
                Span::raw("由于 TUI 捕获了鼠标，复制请按住 "),
                Span::styled("Shift", Style::default().fg(Color::Cyan).bold()),
                Span::raw(" 键再拖拽。"),
            ]),
            Line::from(vec![
                Span::raw(" • "),
                Span::styled("macOS 用户: ", Style::default().bold()),
                Span::raw("请尝试按住 "),
                Span::styled("Option (⌥)", Style::default().fg(Color::Cyan).bold()),
                Span::raw(" 键进行划选。"),
            ]),
        ];

        frame.render_widget(
            Paragraph::new(tips_text)
